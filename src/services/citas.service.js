...
const crearCita = async (datos) => {
  const { Id_cita, nombre, fecha, ID_usuario, ID_notificacion } = datos;
  if (!Id_cita || !nombre || !fecha || !ID_usuario || !ID_notificacion ) {
    throw new Error('Todos los campos son necesarios');
  }

  const nuevoCita = { Id_cita, nombre, fecha, ID_usuario, ID_notificacion };
