...
const crearDetalleProveedor = async (datos) => {
  const { ID_producto, ID_proveedor, observaciones } = datos;
  if (!ID_producto || !ID_proveedor || !observaciones ) {
    throw new Error('Todos los campos son necesarios');
  }

  const nuevoDetalleProveedor = { ID_producto, ID_proveedor, observaciones };
